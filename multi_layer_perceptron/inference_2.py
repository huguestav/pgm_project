...
Y_guess = Y_guess.reshape((height, width, 7))
n_tot = width * height
for i in rand_order[range(n_steps)]:
    ...
    for k in range(nb_labels):
        ...
        L_r_test = sliding_window(a=Y_guess, ws=(8,8,7), ss=(4,4,0), flatten=True)
        L_r_test = L_r_test.reshape((1276,8*8*7))
    ...
    probas_rbm = probas_rbm
    #
    probas = probas_rbm * distrib_mlp[i]
    probas = probas / np.sum(probas)
    # Sample from distribution
    # value = np.random.choice(nb_labels,1,p=probas)[0]
    value = draw_finite(p=probas)
    Y_guess[pixel] = fixed_labels[value]


Y_guess_ = Y_guess.reshape((width*height, 7))
