...
class info:
	...
	def make_regions(self):
	    ...
	    for i in range(self.nb_reg_w):
		p_wt = i * self.reg_incr_w
		p_wb = p_wt + self.reg_w
		for j in range(self.nb_reg_h):
	...
	def __init__(self, database, scale, w, width, height):
		param_dict = {  "corelregional":parameters.param_rbm_corel.reg,
                ...
                params = param_dict[database.lower() + scale]
		...
		self.width = width
		self.height = height
		self.nb_reg_w = (self.width - self.reg_w) / self.reg_incr_w + 1
		self.nb_reg_h = (self.height - self.reg_h) / self.reg_incr_h + 1
		self.nb_reg = self.nb_reg_w * self.nb_reg_h
		self.make_regions()
...
if __name__ == "__main__":
	...
	info_r = info(database, "regional", w_rbm_r, width, height)
